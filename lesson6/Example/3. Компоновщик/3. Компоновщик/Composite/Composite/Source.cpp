...
int main() {
	...
	/*
	comp->Add(new Leaf("10-20 журналов типа «компьютерный мир»", 35));
	root->Add(comp);

	Component* comp1 = new Composite("Мягкий диван", 100);
	root->Add(comp1);

	Component* comp2 = new Composite("Стол секретаря", 40);
	Component* comp3 = new Composite("Компьютер", 80);
	comp3->Add(new Leaf("Большой объем жесткого диска", 25));
	comp2->Add(comp3);
	Component* comp4 = new Composite("Офисный инструментарий", 90);
	comp2->Add(comp4);
	root->Add(comp2);

	Component* comp5 = new Composite("Кулер с теплой и холодной водой", 110);
	root->Add(comp5);


	Component* comp6 = new Composite("Аудитория 1");
	root->Add(comp6);
